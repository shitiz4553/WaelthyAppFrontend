...
function AnalyticsScreen({ navigation }) {
  ...
  function calculateTotalSpentAmount() {
    if (selectedDuration === "week") {
      const activeMonthData = chartData.find(
        (month) => month.month === activeMonth
      );
      if (activeMonthData) {
        const activeWeekData = activeMonthData.weeks.find(
      ...
      if (activeMonthData) {
        return activeMonthData.weeks.reduce(
          (monthSum, week) =>
            monthSum + week.data.reduce((weekSum, value) => weekSum + value, 0),
          0
        );
      }
    }
          ...
          {spendingsData.map((item, index) => {
            ...
            return (
              <AnalyticCard
                ...
                amount={item.amount}
                item={item}
                totalSpentAmount={calculateTotalSpentAmount().toString()} // pass your total amount here..
